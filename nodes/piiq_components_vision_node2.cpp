...
//    cv::Mat kernel = (Mat_<float>(3, 3) << 0.0585, 0.0965, 0.0585, 0.0965, 0.1592, 0.0965, 0.0585, 0.0965, 0.0585);
//    cv::Mat kernel = (Mat_<float>(3, 3) << 0.0585, 0.0965, 0.0585, 0.0965, 0.3, 0.0965, 0.0585, 0.0965, 0.0585);
//      cv::Mat kernel = (Mat_<float>(3, 3) << 0,0, 0, 0, 1.5, 0, 0, 0, 0);
//    cv::filter2D(img_src, img_src, CV_8UC3, kernel);
//    cv::GaussianBlur(img_src, img_src, Size(7, 7),3,3);                         //模糊去噪


    bool res_out;
//    ROS_INFO("!!!!!!");
    res_out = componDetec.onObjDetection(img_src);
//    ROS_INFO("???????!");
...
//       printf("\nCompo Position in world:  %lf  %lf  %lf\n", tfvec_comp_RB.m_floats[0], tfvec_comp_RB.m_floats[1], tfvec_comp_RB.m_floats[2]);
       ...
       pub_pose0.pose.orientation.x= tfqua_comp_RB.z();
       pub_pose0.pose.orientation.y= tfqua_comp_RB.y();
       pub_pose0.pose.orientation.z= tfqua_comp_RB.x();
    ...
    printf("\nAng_x_Z: %lf , Ang_y_Z: %lf , Ang_z_Z: %lf\n", ang_x_Z, ang_y_Z, ang_z_Z);
